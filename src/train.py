...
logger.info('Dividing dataset in train and test...')

X = data.drop(columns=['worldwide_gross'])
...
grid_search = GridSearchCV(estimator=model, param_grid=param_tuning, scoring='r2', cv=5)
...
final_result = cross_validate(grid_search.best_estimator_, X_train, y_train, cv=5, return_train_score=True, scoring='r2')
...
logger.info('Generation model report...')
